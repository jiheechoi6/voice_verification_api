...
class Pretrained:
    ...
    def load_audio_stream(self, data):
        '''Converts latin-1 decoded binary string to normalized tensor form to be processed
        Arguments
        ---------
        data: 
            latin-1 decoded binary string of audio data
        Returns
        -------
        Tensor: normalized audio data
        '''
        # signal, sr = torchaudio.load(data, channels_first=False)
        # normalized = self.audio_normalizer(signal, sr)
        # print(normalized)
        # return normalized 

        import numpy as np
        max = np.iinfo(np.int16).max
        waveform = np.frombuffer(data.encode("latin-1"), dtype=np.int16)/max  # normalize
        # print(waveform)
    ...
    def from_hparams(
        ...
        **kwargs,
        ...
        # run_opts={"device": ...}
        #print(hparams['device'])
        # pretrainer.load_collected(device=hparams['device'])
        pretrainer.load_collected(device='cuda:0')
